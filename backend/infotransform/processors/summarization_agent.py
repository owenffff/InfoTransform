...
class SummarizationAgent:
    ...
    def __init__(self):
        ...
        self.token_threshold = self.config.get(
            "ai_pipeline.summarization.token_threshold", 200000
        )
        self.summary_temperature = self.config.get(
            "ai_pipeline.summarization.temperature", 0.1
        )
        self.summarizing_prompt = self.config.get(
            "ai_pipeline.summarization.prompt", ""
        )
        self.summary_model = self.config.get(
            "ai_pipeline.summarization.model", "vertex_ai.gemini-1.5-pro"
        )
    ...
    def _get_or_create_agent(self, fields: List[str]) -> Agent:
        ...
        # Create agent
        agent = Agent(model, output_type=DocumentSummary, system_prompt=system_prompt)
    ...
    async def summarize_content(
        self, content: str, fields: List[str], filename: str = "document"
    ) -> Dict[str, Any]:
        """
...
"""
            ...
            return {
                ...
                "compression_ratio": len(content) / len(summary_text)
