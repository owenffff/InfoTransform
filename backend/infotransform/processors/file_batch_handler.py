...
class BatchProcessor:
            ...
            with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                ...
                for file_info in zip_ref.filelist:
                    if not file_info.is_dir():
                        ...
                        if not relative_path.startswith('.') and not relative_path.startswith('__'):
                            files_info.append({
                                'path': relative_path,
                                'full_path': full_path,
    ...
    async def process_file_async(self, processor: Any, file_path: str, relative_path: str) -> Dict[str, Any]:
    ...
    async def process_multiple_files(self, files_info: List[Dict[str, str]]) -> Dict[str, Any]:
        """
    ...
    def create_combined_markdown(self, results: List[Dict[str, Any]]) -> str:
        ...
        for i, result in enumerate(results):
            if result.get('success'):
                relative_path = result.get('relative_path', result.get('filename', 'Unknown'))
    ...
    def create_zip_archive(self, results: List[Dict[str, Any]], output_path: str) -> str:
        """
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for result in results:
                if result.get('success'):
                    relative_path = result.get('relative_path', result.get('filename', 'unknown.md'))
                    ...
                    with open(full_md_path, 'w', encoding='utf-8') as f:
                        ...
                        f.write(f"**Processed Type**: {result.get('type', 'unknown')}\n\n")
