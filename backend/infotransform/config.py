...
class Config:
    def __init__(self):
        ...
        if not config_path.exists():
            logger.info(
                f"Environment-specific config not found: {config_path}, using default config.yaml"
            )
        ...
        cache_config_path = (
            Path(__file__).parent.parent.parent / "config" / "caching.yaml"
        )
        if cache_config_path.exists():
            with open(cache_config_path, "r", encoding="utf-8") as f:
                self.cache_config = yaml.safe_load(f)
                logger.info(f"Loaded caching configuration from {cache_config_path}")
        else:
            logger.info("Caching config not found, using defaults")
            self.cache_config = self._get_default_cache_config()
    ...
    def get(self, key_path: str, default: Any = None) -> Any:
        ...
        Searches in order: main config, cache config, performance config
        ...
        # Try cache config second
        value = self.cache_config
        for key in keys:
            if isinstance(value, dict) and key in value:
                value = value[key]
            else:
    ...
    def PORT(self):
        ...
        port_str = os.getenv(
            "BACKEND_PORT", os.getenv("PORT", str(self.get("api.port", 8000)))
        ).strip()
        try:
            return int(port_str)
        except ValueError:
    ...
    def get_analysis_prompt(self, model_key: Optional[str] = None) -> str:
        ...
        if model_key:
            model_prompt = self.get(
                f"ai_pipeline.structured_analysis.prompts.{model_key}"
            )
            if model_prompt:
                return model_prompt
    ...
    @property
    def MAX_TOTAL_UPLOAD_SIZE(self):
        return (
            self.get("processing.upload.max_total_upload_size_mb", 1024) * 1024 * 1024
        )
    ...
    def _get_default_cache_config(self) -> Dict[str, Any]:
        ...
        return {
            "result_cache": {
                ...
                "invalidation_strategy": "ttl_only",
            },
            "monitoring": {
                "enable_metrics": True,
    ...
    def validate(self):
        ...
        for section in required_ai_sections:
            if section not in ai_pipeline:
                raise ValueError(
                    f"Missing required AI pipeline section '{section}' in config.yaml"
                )
...
MAX_TOTAL_UPLOAD_SIZE = config.MAX_TOTAL_UPLOAD_SIZE
