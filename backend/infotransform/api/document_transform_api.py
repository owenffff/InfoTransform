...
class StreamingProcessor:
    ...
    def _extract_zip_recursive(
        ...
        temp_dir = tempfile.mkdtemp(
            prefix="infotransform_zip_", dir=config.TEMP_EXTRACT_DIR
        )
    ...
    def _get_display_fields(self, structured_data: Dict[str, Any]) -> List[str]:
        ...
        if (
            len(structured_data) == 1
            and "item" in structured_data
            and isinstance(structured_data["item"], list)
            and len(structured_data["item"]) > 0
            and isinstance(structured_data["item"][0], dict)
        ):
    ...
    def _expand_nested_results(
        self, ai_result: Dict[str, Any], original_item: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
        ...
        if (
            ...
            for idx, item_data in enumerate(items):
                ...
                expanded_result = {
                    ...
                    "model_fields": list(item_data.keys())
        ...
        ai_result["display_name"] = original_item.get(
            "display_name", ai_result["filename"]
        )
    ...
    async def process_files_optimized(
        ...
        for file_info in files:
            if self._is_zip_file(file_info["filename"]):
                ...
                extracted = self._extract_zip_recursive(
                    file_info["file_path"], file_info["filename"]
                )
                expanded_files.extend(extracted)
                logger.info(
                    f"Expanded ZIP {file_info['filename']}: {len(extracted)} files"
                )
            else:
        ...
        if zip_count > 0:
            logger.info(
                f"[{run_id}] Processed {zip_count} ZIP files, total files: {len(expanded_files)}"
            )
        ...
        logger.info(
            f"[{run_id}] Starting processing run: {total_files} files, model={model_key}, ai_model={ai_model}"
        )

        # Get model information
        model_info = self.structured_analyzer_agent.get_available_models().get(
            model_key, {}
        )
        ...
        async with self.file_manager.batch_context(files) as managed_files:
            ...
            logger.info(
                f"[{run_id}] Markdown conversion complete: {len(files)} files in {conversion_time:.2f}s"
            )
            phase_complete_event = {
                ...
                "files_per_second": len(files) / conversion_time
            ...
            if successful_conversions:
                ...
                logger.info(
                    f"[{run_id}] AI processing complete: {len(successful_conversions)} files in {ai_time:.2f}s"
                )
                ai_complete_event = {
                    "type": "phase",
                    "phase": "ai_processing",
                    "status": "completed",
                    "duration": ai_time,
                    "files_per_second": len(successful_conversions) / ai_time
                    if ai_time > 0
                    else 0,
                }
            ...
            for failed in failed_conversions:
                failed_result_event = {
                    ...
                    "file_path": failed.get(
                        "file_path"
                    ),  # Add file path for review session
                    "progress": {
                        "current": len(successful_conversions)
                        + failed_conversions.index(failed)
                        + 1,
                        "total": total_files,
                        "successful": successful_ai,
                        "failed": failed_ai + len(failed_conversions),
            ...
            summarized_count = sum(
                1
                for item in successful_conversions
                if item.get("was_summarized", False)
            )
            summarization_time = (
                summarization_time if "summarization_time" in locals() else 0
            )

            # Get batch metrics with token usage and cache stats
            batch_metrics = self.batch_processor.get_metrics()
            token_usage = batch_metrics.get("token_usage", {})
            cache_metrics = batch_metrics.get("cache", {})
            ...
            completion_event = {
                ...
                or config.get("models.ai_models.default_model"),
                "token_usage": {
                ...
                "cache_stats": cache_metrics,  # Include cache performance metrics
                "summarization": {
...
async def transform(
    ...
    try:
        for file in files:
            ...
            file_infos.append({"file_path": file_path, "filename": file.filename})
    ...
    managed_response = ManagedStreamingResponse(
        processor.process_files_optimized(
            file_infos, model_key, custom_instructions, ai_model, run_id=run_id
        ),
        saved_files,
