...
class FileLifecycleManager:
    ...
    def __init__(self):
        ...
        self.max_retention = float(config.get_performance(
            'file_management.max_file_retention', 300
        ))
        self.cleanup_interval = float(config.get_performance(
            'file_management.cleanup_check_interval', 10
        ))
    ...
    async def acquire_file(self, file_path: str) -> None:
        ...
        async with self.file_locks[file_path]:
            ...
            logger.debug(f"Acquired file {file_path}, refs: {self.file_refs[file_path]}")
    ...
    async def release_file(self, file_path: str) -> None:
        ...
        async with self.file_locks[file_path]:
            if file_path in self.file_refs:
                ...
                logger.debug(f"Released file {file_path}, refs: {self.file_refs[file_path]}")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'pending_cleanup': len([
                f for f, e in self.cleanup_events.items() 
                if e.is_set()
            ]),
            'oldest_file_age': max([
                (datetime.now() - t).total_seconds()
                for t in self.file_creation_times.values()
            ]) if self.file_creation_times else 0
        }
