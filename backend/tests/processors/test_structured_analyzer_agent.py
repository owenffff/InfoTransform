...
class TestStructuredAnalyzerAgent:
    ...
    def mock_available_models(self):
        ...
        return {"invoice": MockInvoiceModel}

    @pytest.fixture
    def analyzer(self, mock_available_models):
        """Create analyzer with mocked models"""
        with patch(
            "infotransform.processors.structured_analyzer_agent.AVAILABLE_MODELS",
            mock_available_models,
        ):
            from infotransform.processors.structured_analyzer_agent import (
    ...
    async def test_analyze_content_success(
        self, mock_agent_class, analyzer, sample_markdown_content
    ):
    ...
    @pytest.mark.asyncio
    async def test_analyze_content_invalid_model(
        self, analyzer, sample_markdown_content
    ):
        """Test analysis with invalid model key"""
        result = await analyzer.analyze_content(
            sample_markdown_content, "nonexistent_model"
        )
    ...
    @pytest.mark.asyncio
    @patch("infotransform.processors.structured_analyzer_agent.Agent")
    async def test_analyze_content_with_exception(
        self, mock_agent_class, analyzer, sample_markdown_content
    ):
        ...
        result = await analyzer.analyze_content(sample_markdown_content, "invoice")
    ...
    @pytest.mark.asyncio
    @patch("infotransform.processors.structured_analyzer_agent.Agent")
    async def test_analyze_content_stream(
        self, mock_agent_class, analyzer, sample_markdown_content
    ):
        ...
        async for result in analyzer.analyze_content_stream(
            sample_markdown_content, "invoice"
        ):
    ...
    def test_get_or_create_agent_caching(self, analyzer):
        ...
        with patch(
            "infotransform.processors.structured_analyzer_agent.Agent"
        ) as mock_agent_class:
            mock_agent_class.return_value = MagicMock()

            # Create agent first time
            agent1 = analyzer._get_or_create_agent(
                MockInvoiceModel, "invoice", "gpt-4o"
            )

            # Create agent second time - should use cache
            agent2 = analyzer._get_or_create_agent(
                MockInvoiceModel, "invoice", "gpt-4o"
            )
...
class TestAnalyzerConfiguration:
    ...
    @pytest.mark.asyncio
    @patch("infotransform.processors.structured_analyzer_agent.Agent")
    async def test_analyzer_respects_model_settings(
        self, mock_agent_class, sample_markdown_content
    ):
        ...
        with patch(
            "infotransform.processors.structured_analyzer_agent.AVAILABLE_MODELS",
            {"test": MockInvoiceModel},
        ):
            result = await analyzer.analyze_content(
                sample_markdown_content, "test", ai_model="gpt-4o"
            )
            ...
            assert result is not None  # Verify result variable is used
